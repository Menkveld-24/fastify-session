...
var Session = class _Session {
  ...
  static async fromCookie(cookie) {
    const { buffer: cleartext, rotated } = _Session.#sessionCrypto.unsealMessage(cookie, _Session.#secretKeys);
    if (_Session.#sessionCrypto.stateless) {
      return await _Session.fromStatelessCookie(cleartext.toString(), rotated);
    }
    return await _Session.fromStatefulCookie(cleartext.toString(), rotated);
  }
  /**
  ...
  static async fromStatelessCookie(payload, rotated) {
    ...
    const session = await _Session.create(data, { id: data.id });
  ...
  async toCookie() {
    const buffer = Buffer.from(
      _Session.#sessionCrypto.stateless ? JSON.stringify({ ...this.#sessionData, id: this.id }) : this.id
    );
  ...
  async reload(request) {
    ...
    (0, import_node_assert.default)(_Session.#sessionStore);
    const session = await _Session.fromStatefulCookie(this.id, this.rotated);
...
var plugin = async (fastify, options = {}) => {
  ...
  fastify.decorate("loadSession", async (request) => {
    let cookies;
    let log;
    if (request instanceof import_http.IncomingMessage) {
      if (request.headers.cookie === void 0)
        return;
      log = fastify.log;
      cookies = (0, import_cookie.parse)(request.headers.cookie);
    } else {
  ...
  async function decodeRequestCookie(request, cookies, log) {
  ...
  async function encodeSession(request, log) {
